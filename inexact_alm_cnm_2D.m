function [X] = inexact_alm_cnm_2D(Y, G,  ksize, lambda, tol, maxIter)
%%% This matlab code implements the (inexact) augmented Lagrange multiplier method for
%%% min_{x}  |A(X)|_* + 0.5*ksize(1)*ksize(2)*lambda |P_Omega(x - y)|_2^2
%%% A(X) -- linear operator that returns the convolutional matrix of X
%%% | |_* -- the nuclear norm of a matrix
%% parameters:
% Y -- m x n vector of observations (required input),with the missing entries being filled with zero
% G -- m x n vector with 1 and 0 denoting the observed and missing entries respecitvely (required input) 
...
fnorm = sqrt(k1*k2)*norm(Y,'fro');

% initialize
W = zeros(m*n,k1*k2); %Lagrange multiplier 
X = Y;

%parameters
obsrate = sum(G(:)>0.5)/(m*n);
mu = obsrate/fnorm; % this one can be tuned
rho = 1.05; % this one should be chosen carefully

iter = 0;
%% loop
Ax = cconv2mtx(X,ksize);
while iter < maxIter       
   iter = iter + 1;   
    %update L
    temp = Ax + W./mu;
    ...
    %update x
    temp = adj_2D((L - W./mu)./(k1*k2),[m n],ksize);
